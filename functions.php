...
function getCounts() {
    ...
    // Consultas SQL para contar clientes, mesas y órdenes
    $queryClientes = "SELECT COUNT(*) AS totalClientes FROM tbl_customer";
    $queryMesas = "SELECT COUNT(*) AS totalMesas FROM tbl_table";
    $queryOrdenes = "SELECT COUNT(*) AS totalOrdenes FROM tbl_order";
	$queryproduct = "SELECT COUNT(*) AS totalproduct FROM tbl_menuitem";
	$querycategory = "SELECT COUNT(*) AS totalcategory FROM tbl_menu";
	$querystaff = "SELECT COUNT(*) AS totalstaff FROM tbl_staff";

    $counts = [];

    // Ejecutar la consulta para contar clientes
    if ($result = $sqlconnection->query($queryClientes)) {
        $row = $result->fetch_array(MYSQLI_ASSOC);
        $counts['clientes'] = $row['totalClientes'];
    } else {
        echo "Error al contar clientes: " . $sqlconnection->error;
        return null;
    }

    // Ejecutar la consulta para contar mesas
    if ($result = $sqlconnection->query($queryMesas)) {
        $row = $result->fetch_array(MYSQLI_ASSOC);
        $counts['mesas'] = $row['totalMesas'];
    } else {
        echo "Error al contar mesas: " . $sqlconnection->error;
        return null;
    }

    // Ejecutar la consulta para contar órdenes
    if ($result = $sqlconnection->query($queryOrdenes)) {
        $row = $result->fetch_array(MYSQLI_ASSOC);
        $counts['ordenes'] = $row['totalOrdenes'];
    } else {
        echo "Error al contar órdenes: " . $sqlconnection->error;
        return null;
    }
  // Ejecutar la consulta para contar productos
  if ($result = $sqlconnection->query($queryproduct)) {
	$row = $result->fetch_array(MYSQLI_ASSOC);
	$counts['product'] = $row['totalproduct'];
} else {
	echo "Error al contar product: " . $sqlconnection->error;
	return null;
}
  // Ejecutar la consulta para contar categorias
  if ($result = $sqlconnection->query($querycategory)) {
	$row = $result->fetch_array(MYSQLI_ASSOC);
  ...
  if ($result = $sqlconnection->query($querystaff)) {
	$row = $result->fetch_array(MYSQLI_ASSOC);
